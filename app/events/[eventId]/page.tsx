...
type PlanKey = "lake0" | "lake1" | "wc" | "coach";
...
function friendlyAptLabel(aptId: string) {
  ...
  if (aptId === "apt_ch") return "Coach House";
...
function startOfIsoDate(iso: string) {
  const [y, m, d] = iso.split("-").map(Number);
  return new Date(y, (m || 1) - 1, d || 1);
}

function diffDays(fromIso: string, toIso: string) {
  const from = startOfIsoDate(fromIso);
  const to = startOfIsoDate(toIso);
  const ms = to.getTime() - from.getTime();
  return Math.round(ms / 86400000);
}

function checkinLabel(dateIso: string, eventStartIso: string) {
  const daysBefore = diffDays(dateIso, eventStartIso);
  if (daysBefore <= 0) return `${dateIso} (event day)`;
  if (daysBefore === 1) return `${dateIso} (one day before)`;
  if (daysBefore === 2) return `${dateIso} (two days before)`;
  return `${dateIso} (${daysBefore} days before)`;
}

function buildCheckinOptions(eventStartIso: string, arrivalStartIso: string) {
  if (!eventStartIso) return [];
  const start = startOfIsoDate(eventStartIso);
  const arrival = arrivalStartIso ? startOfIsoDate(arrivalStartIso) : start;
  const first = arrival <= start ? arrival : start;
  const options: { value: string; label: string }[] = [];
  const cursor = new Date(first);
  while (cursor <= start) {
    const value = isoDate(cursor);
    options.push({ value, label: checkinLabel(value, eventStartIso) });
...
export default function EventPlannerPage() {
  ...
  const [svgs, setSvgs] = useState<Record<PlanKey, string>>({ lake0: "", lake1: "", wc: "", coach: "" });
  ...
  const [arrivalStartDate, setArrivalStartDate] = useState<string>("");
  ...
  const checkinOptions = useMemo(() => buildCheckinOptions(eventStart, arrivalStartDate || eventStart), [eventStart, arrivalStartDate]);
    ...
    (async () => {
      try {
        ...
        const { data: ev, error: evErr } = await supabase
          .from("events")
          .select("status,start_date,end_date,arrival_start_date,couple_note,portal_feedback_rating,portal_feedback_comment")
          .eq("id", eventId)
          .single();
        ...
        const arrival = ((ev as any)?.arrival_start_date ?? s ?? "") as string;
        setEventStart(s);
        setEventEnd(e);
        setArrivalStartDate(arrival || s || "");
        setCheckinDate(s || "");
        setCheckoutDate(e || "");
        ...
        if (!svgs.lake0) {
          const [s0, s1, sw, sc] = await Promise.all([
            loadSvg("/plans/lakehouse_0floor.svg"),
            loadSvg("/plans/lakehouse_1floor.svg"),
            loadSvg("/plans/woodcutter_0floor.svg"),
            loadSvg("/plans/coachhouse_0floor.svg"),
          ]);
          setSvgs({ lake0: s0, lake1: s1, wc: sw, coach: sc });
        }
      } catch (e: any) {
            ...
            <div className="card card-pad planner-add-card">
              ...
              <div className="planner-form-grid">
                ...
                <div>
                  ...
                  <select value={checkinDate || eventStart} disabled={saving || locked || checkinOptions.length === 0} onChange={(e) => setCheckinDate(e.target.value)}>
            ...
            <div className="grid-main">
              <div className="card card-pad">
                <div className="tabs" style={{ marginBottom: 12 }}>
                  ...
                  <button className={`tab ${activePlan === "coach" ? "active" : ""}`} onClick={() => setActivePlan("coach")}>Coach House</button>
                </div>
            ...
            <div className="modal guest-edit-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body">
                ...
                <div className="guest-edit-form">
                  ...
                  <div>
                    <div className="label">Check-in</div>
                    <select value={editingGuest.checkin_date ?? eventStart} onChange={(e) => updateEditingGuest("checkin_date", e.target.value || null)} disabled={editSaving || checkinOptions.length === 0}>
                      {checkinOptions.map((option) => (
                        <option key={option.value} value={option.value}>{option.label}</option>
                      ))}
                    </select>
                    ...
                    {forgottenGuestOpen && (
                      <div className="forgotten-guest-form">
                        ...
                        <div className="planner-form-grid" style={{ marginTop: 10 }}>
                          ...
                          <div><div className="label">Check-in</div><select value={checkinDate || eventStart} disabled={saving || locked || checkinOptions.length === 0} onChange={(e) => setCheckinDate(e.target.value)}>{checkinOptions.map((option) => (<option key={option.value} value={option.value}>{option.label}</option>))}</select></div>
