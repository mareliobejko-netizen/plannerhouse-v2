...
function fmtDate(d: string | null) {
  ...
  return new Intl.DateTimeFormat("en-GB", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(`${d}T12:00:00`));
}
...
export default function AdminEventsPage() {
  ...
  async function createUserAndEvent() {
    ...
    if (password.length < 6) {
      setCreateMsg({ type: "err", text: "Password is too short (minimum 6 characters)." });
      return;
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (["draft", "submitted", "final"].includes(statusFilter)) {
      ...
      data = data.filter((ev) => !ev.start_date || new Date(`${ev.start_date}T00:00:00`) >= today);
    } else if (statusFilter === "past") {
      data = data.filter((ev) => ev.end_date && new Date(`${ev.end_date}T23:59:59`) < today);
    }
    ...
    data.sort((a, b) => {
      ...
      const ad = a.start_date ? new Date(`${a.start_date}T00:00:00`).getTime() : Number.MAX_SAFE_INTEGER;
      const bd = b.start_date ? new Date(`${b.start_date}T00:00:00`).getTime() : Number.MAX_SAFE_INTEGER;
      return ad - bd;
    });
