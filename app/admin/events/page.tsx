...
export default function AdminEventsPage() {
  ...
  const [newArrivalStart, setNewArrivalStart] = useState("");
    ...
    try {
      ...
      const res = await fetch("/api/admin/create-user-and-event", {
        ...
        body: JSON.stringify({
          ...
          arrival_start_date: newArrivalStart || newStart || null,
          ...
          <div className="admin-create-modal" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="admin-create-body">
              ...
              <div className="admin-create-section">
                ...
                <div className="admin-form-grid">
                  <div><div className="label">Start date</div><input className="input" type="date" value={newStart} onChange={(e) => { const value = e.target.value; setNewStart(value); setNewArrivalStart((current) => (!current || current === newStart ? value : current)); }} disabled={creating} /></div>
                  ...
                  <div><div className="label">Arrival date</div><input className="input" type="date" value={newArrivalStart || newStart} max={newStart || undefined} onChange={(e) => setNewArrivalStart(e.target.value)} disabled={creating || !newStart} /></div>
                </div>
                <p className="muted" style={{ marginTop: 10 }}>Set the earliest date guests are allowed to arrive. If you leave it unchanged, it stays on the event start date.</p>
              </div>
