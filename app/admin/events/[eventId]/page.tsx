...
export default function AdminEventPage() {
  ...
  const [settingsArrivalStart, setSettingsArrivalStart] = useState("");
  ...
  async function load() {
    ...
    try {
      ...
      const [eventResult, occupancyResult, guestsResult] = await Promise.all([
        supabase
          .from("events")
          .select("id,name,status,start_date,end_date,arrival_start_date,created_at,created_by,submitted_at,submitted_by,welcome_title,welcome_message,tip_message,tutorial_video_url,couple_note,portal_feedback_rating,portal_feedback_comment")
          .eq("id", eventId)
          .single(),
        supabase
      ...
      setSettingsArrivalStart(eventData.arrival_start_date ?? eventData.start_date ?? "");
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          arrival_start_date: settingsArrivalStart || settingsStart || null,
        ...
        {activeTab === "settings" && (
          <section className="card card-pad admin-event-panel admin-event-settings-panel">
            ...
            <div className="admin-event-settings-grid">
              ...
              <div><label>Start date</label><input className="input" type="date" value={settingsStart} onChange={(event) => { const value = event.target.value; setSettingsStart(value); setSettingsArrivalStart((current) => (!current || current === settingsStart ? value : current)); }} /></div>
              ...
              <div><label>Arrival date</label><input className="input" type="date" value={settingsArrivalStart || settingsStart} max={settingsStart || undefined} onChange={(event) => setSettingsArrivalStart(event.target.value)} /></div>
