...
export async function POST(req: Request) {
  try {
    ...
    if (body.arrival_start_date && body.start_date && body.arrival_start_date > body.start_date) {
      return NextResponse.json({ error: "Arrival date cannot be after the event start date" }, { status: 400 });
    }
    ...
    try {
      ...
      const eventRows = await sql`
        INSERT INTO public.events (
          ...
          ${body.arrival_start_date ?? body.start_date ?? null}::date,
          ${newUserId}::uuid,
          'draft',
